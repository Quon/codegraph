...
class ToolHandler {
    ...
    resolveProjects(project, projectPath) {
        ...
        if (!project || project === '.') {
            ...
            if (!this.cg)
                throw new Error('CodeGraph not initialized for this project.');
            return new Map([['.', this.cg]]);
        }
        // Resolve from registry
        if (!this.projectRoot)
            throw new Error('No project root configured. Cannot resolve sub-projects.');
        const projects = (0, index_1.loadProjects)(this.projectRoot);
        if (project === '*') {
            // Open all registered projects
            const map = new Map();
            map.set('.', this.cg);
            for (const name of projects) {
                const absPath = (0, path_1.resolve)(this.projectRoot, name);
        ...
        const absPath = (0, path_1.resolve)(this.projectRoot, project);
        if (!(0, utils_1.validatePathWithinRoot)(this.projectRoot, absPath)) {
            throw new Error(`Project path "${project}" is outside the project root`);
        }
        const cached = this.projectCache.get(absPath);
        if (cached)
            return new Map([[project, cached]]);
        const subCg = index_1.default.openSync(absPath);
        this.projectCache.set(absPath, subCg);
    ...
    startWatcherFor(name, absPath, cg) {
        if (this.watchers.has(absPath))
            return;
        const watcher = new sync_1.FileWatcher(absPath, cg.getConfig(), async () => {
            ...
            onSyncComplete: (result) => {
                if (result.filesChanged > 0) {
                    process.stderr.write(`[CodeGraph MCP] Auto-synced "${name}" — ${result.filesChanged} file(s) in ${result.durationMs}ms\n`);
                }
            },
            onSyncError: (err) => {
    ...
    async handleProjects(args) {
        ...
        const projectsList = (0, index_1.loadProjects)(this.projectRoot);
        const projectEntries = [];
        for (const name of projectsList) {
            const absPath = (0, path_1.resolve)(this.projectRoot, name);
            const entry = { name, path: absPath, initialized: (0, index_1.isInitialized)(absPath) };
